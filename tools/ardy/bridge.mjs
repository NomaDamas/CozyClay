...
// ---------------------------------------------------------------------------
// /ardy/inbetween — fill between IK-keyed poses with the distilled Cascadeur
// inbetweener (cozy-inbetween demo/server.py on the box, reached through an
// ssh -L tunnel). Dev-only experiment: the keyed frames are hard, a context
// window around them is regenerated by the model, the rest of the take is
// copied through untouched. Result is registered like any generated take.
// ---------------------------------------------------------------------------
const INBETWEEN_URL = process.env.CCLAY_INBETWEEN_URL || "http://127.0.0.1:7861/api/inbetween";
const INBETWEEN_MAX_WINDOW = 120;
const INBETWEEN_MAX_CONTEXT = 60;
const PUBLIC_DEMO_DIR = resolve(HERE, "../../public/demo");
const CSKEL27_REST_PATH = resolve(HERE, "../../public/ardy/cskel27-rest.json");
let cskel27RestCache = null;
function cskel27Rest() {
	if (!cskel27RestCache) cskel27RestCache = JSON.parse(readFileSync(CSKEL27_REST_PATH, "utf8"));
	return cskel27RestCache;
}
function resolveSourceMotionPath(url) {
	if (typeof url !== "string") return null;
	if (url.startsWith("/ardy/motions/")) {
		const id = url.slice("/ardy/motions/".length);
		return MOTION_ID.test(id) ? motionAllowlist.get(id) || null : null;
	}
	// Staged demo clips (public/demo/*.npz, served by vite) are accepted too so
	// the experiment can run without a fresh Kimodo take.
	const demo = /^\/demo\/([A-Za-z0-9._-]+\.npz)(?:\?.*)?$/.exec(url);
	if (demo && !demo[1].includes("..")) return join(PUBLIC_DEMO_DIR, demo[1]);
	return null;
}
function frameAsReference(motion, frame) {
	const posed_joints = [];
	const local_rot_mats = [];
	for (let j = 0; j < 27; j += 1) {
		const o = (frame * 27 + j) * 3;
		posed_joints.push([motion.posedJoints[o], motion.posedJoints[o + 1], motion.posedJoints[o + 2]]);
		const m = (frame * 27 + j) * 9;
		local_rot_mats.push([
			[motion.rotMats[m], motion.rotMats[m + 1], motion.rotMats[m + 2]],
			[motion.rotMats[m + 3], motion.rotMats[m + 4], motion.rotMats[m + 5]],
			[motion.rotMats[m + 6], motion.rotMats[m + 7], motion.rotMats[m + 8]],
		]);
	}
	return { schema: "ardy.frame.v1", posed_joints, local_rot_mats };
}
const clampInt = (value, fallback, lo, hi) => (Number.isInteger(value) ? Math.min(hi, Math.max(lo, value)) : fallback);
const meanOf = (list) => (Array.isArray(list) && list.length ? list.reduce((a, b) => a + b, 0) / list.length : 0);
...
async function handleInbetween(req, res) {
	...
	if (edits.length < 1 || edits.length > 64) {
		sendJson(res, 400, { ok: false, reason: "field 'edits' must have 1..64 entries" });
		return;
	}
	for (const [i, entry] of edits.entries()) {
		if (!entry || !Number.isInteger(entry.frame) || entry.frame < 0) {
			sendJson(res, 400, { ok: false, reason: `edits[${i}].frame must be a non-negative integer` });
			return;
		}
		if (entry.pose?.schema !== "cozyclay.pose.v1") {
			sendJson(res, 400, { ok: false, reason: `edits[${i}].pose.schema must be "cozyclay.pose.v1"` });
			return;
		}
	}
	edits.sort((a, b) => a.frame - b.frame);
	const rest = cskel27Rest();
	const fitPose = (entry, reference) => poseToCskel27({ pose: entry.pose, rest, reference });
	const writeFrame = (rot, pj, f, fitted) => {
		for (let j = 0; j < 27; j += 1) {
			const m = fitted.local_rot_mats[j];
			rot.set([m[0][0], m[0][1], m[0][2], m[1][0], m[1][1], m[1][2], m[2][0], m[2][1], m[2][2]], (f * 27 + j) * 9);
			pj.set(fitted.posed_joints[j], (f * 27 + j) * 3);
		}
	};
	...
	let pj;
	let keys;
	const fromScratch = body.sourceMotion === null || body.sourceMotion === undefined;
	if (fromScratch) {
		// No clip underneath: build one. Every frame before the first key holds
		// the first pose, every frame after the last key holds the last pose,
		// frames between keys start as a hold of the previous key (the model
		// overwrites them). Outside-frames are sent as keys so they stay put.
		T = Number.isInteger(body.frames) ? body.frames : 0;
		const last = edits[edits.length - 1].frame;
		if (T < 4 || T > INBETWEEN_MAX_WINDOW || last >= T) {
			sendJson(res, 400, { ok: false, reason: `field 'frames' must be 4..${INBETWEEN_MAX_WINDOW} and beyond the last key (${last})` });
			return;
		}
		if (edits.length < 2) {
			sendJson(res, 400, { ok: false, reason: "at least two keyed poses are needed without a source motion" });
			return;
		}
		s = 0;
		rot = new Float32Array(T * 243);
		pj = new Float32Array(T * 81);
		let fitted;
		try {
			fitted = edits.map((entry) => fitPose(entry, undefined));
		} catch (err) {
			sendJson(res, 400, { ok: false, reason: `pose conversion: ${err.message}` });
			return;
		}
		let k = 0;
		for (let f = 0; f < T; f += 1) {
			while (k + 1 < edits.length && edits[k + 1].frame <= f) k += 1;
			writeFrame(rot, pj, f, fitted[k]);
		}
		const first = edits[0].frame;
		keys = [];
		for (let f = 0; f < T; f += 1) if (f <= first || f >= last) keys.push(f);
		for (const entry of edits) if (!keys.includes(entry.frame)) keys.push(entry.frame);
		keys.sort((a, b) => a - b);
	} else {
		const sourcePath = resolveSourceMotionPath(body.sourceMotion);
		if (!sourcePath || !existsSync(sourcePath)) {
			sendJson(res, 400, { ok: false, reason: `field 'sourceMotion': unknown or expired motion ${JSON.stringify(body.sourceMotion)}` });
			return;
		}
		base = await decodeMotionNpz(readFileSync(sourcePath));
		const { startFrame, endFrame } = body;
		if (!Number.isInteger(startFrame) || !Number.isInteger(endFrame) || startFrame < 0 || endFrame < startFrame || endFrame >= base.frames) {
			sendJson(res, 400, { ok: false, reason: `startFrame/endFrame must satisfy 0 <= start <= end < ${base.frames}` });
			return;
		}
		if (edits.some((entry) => entry.frame < startFrame || entry.frame > endFrame)) {
			sendJson(res, 400, { ok: false, reason: `every edit frame must lie in ${startFrame}..${endFrame}` });
			return;
		}
		const before = clampInt(body.contextBefore, 12, 0, INBETWEEN_MAX_CONTEXT);
		const after = clampInt(body.contextAfter, 12, 0, INBETWEEN_MAX_CONTEXT);
		s = Math.max(0, startFrame - before);
		const e = Math.min(base.frames - 1, endFrame + after);
		T = e - s + 1;
		if (T < 4 || T > INBETWEEN_MAX_WINDOW) {
			sendJson(res, 400, { ok: false, reason: `window ${s}..${e} is ${T} frames; the inbetweener takes 4..${INBETWEEN_MAX_WINDOW}` });
			return;
		}
		rot = new Float32Array(base.rotMats.subarray(s * 243, (e + 1) * 243));
		pj = new Float32Array(base.posedJoints.subarray(s * 81, (e + 1) * 81));
		for (const entry of edits) {
			let fitted;
			try {
				fitted = fitPose(entry, frameAsReference(base, entry.frame));
			} catch (err) {
				sendJson(res, 400, { ok: false, reason: `edits frame ${entry.frame}: ${err.message}` });
				return;
			}
			writeFrame(rot, pj, entry.frame - s, fitted);
		}
		keys = [...new Set([0, T - 1, ...edits.map((entry) => entry.frame - s)])].sort((a, b) => a - b);
	}
	const nested = (flat, dims) => {
		const walk = (offset, depth) => {
			if (depth === dims.length - 1) return Array.from(flat.subarray(offset, offset + dims[depth]));
			const stride = dims.slice(depth + 1).reduce((a, b) => a * b, 1);
			const out = [];
			for (let i = 0; i < dims[depth]; i += 1) out.push(walk(offset + i * stride, depth + 1));
			return out;
		};
		return walk(0, 0);
	};
	const payload = {
		local_rot_mats: nested(rot, [T, 27, 3, 3]),
		posed_joints: nested(pj, [T, 27, 3]),
		keys,
		style: clampInt(body.style, 14, 0, 14),
		tag: body.model === "orig" ? "orig" : body.model === "root" ? "root" : "v3",
		seed: Number.isInteger(body.seed) ? body.seed : 0,
	};
	let out;
	try {
		const reply = await fetch(INBETWEEN_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
		out = await reply.json();
		if (!reply.ok || out.error) throw new Error(out.error || `HTTP ${reply.status}`);
	} catch (err) {
		sendJson(res, 502, { ok: false, reason: `inbetweener at ${INBETWEEN_URL} failed: ${err.message} (is the box demo server + ssh tunnel up?)` });
		return;
	}
	const fps = base ? base.fps : clampInt(body.fps, 24, 1, 120);
	const replacement = {
		frames: T,
		fps,
		rotMats: Float32Array.from(out.local_rot_mats.flat(3)),
		rootPos: Float32Array.from(out.root_positions.flat()),
		posedJoints: Float32Array.from(out.posed_joints.flat(2)),
	};
	const edited = base ? replaceMotionSegment(base, replacement, s) : replacement;
	const stamp = `${Date.now()}-${randomBytes(3).toString("hex")}`;
	const artifactDir = createPrivateArtifactDir(OUT_DIR, "inbetween");
	const outNpzPath = join(artifactDir, "inbetween.npz");
	writeNpz(outNpzPath, motionArraysToNpzMembers(edited));
	registerMotion(stamp, outNpzPath);
	const e = s + T - 1;
	console.log(`[bridge] inbetween${fromScratch ? " (from scratch)" : ""} model=${payload.tag} boxMs=${JSON.stringify(out.ms)}: frames ${s}..${e} (${T}) keys ${keys.length <= 12 ? keys.map((k) => k + s).join(",") : `${keys.length} frames`} fit ${meanOf(out.fit_err_cm).toFixed(2)} cm in ${Date.now() - started} ms`);
	sendJson(res, 200, {
		ok: true,
		motionUrl: `/ardy/motions/${stamp}`,
		window: [s, e],
		keys: keys.map((k) => k + s),
		fitErrCm: meanOf(out.fit_err_cm),
		modelVsInputCm: meanOf(out.model_vs_input_cm),
		ms: Date.now() - started,
		boxMs: out.ms,
	});
}
