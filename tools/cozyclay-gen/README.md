...
`$COMFY_DATA/models`, and one 96 GB-class GPU. Prompt conversion uses a user-provided OpenAI-compatible API key/model (BYOK).
...
Set `COZYCLAY_LLM_BASE_URL`, `COZYCLAY_LLM_API_KEY`, and `COZYCLAY_LLM_MODEL` for the server-wide default, or enter them in the form's advanced settings.

Then open `http://<host>:8288/` (plain form). The developer node canvas is at `/graph`; `/form` redirects to `/` for compatibility.
...
`Scene to Mocap Prompt (AI)`: type the scene in plain Korean/English; a BYOK OpenAI-compatible model applies the `h3-mocap-prompting` skill (embedded as its system prompt in `rewriter.py`) and writes the full Ref2VA contract. `POST /rewrite {"scene": "...", "seconds": 15, "llm": {"base_url", "api_key", "model"}}` accepts per-request settings.
