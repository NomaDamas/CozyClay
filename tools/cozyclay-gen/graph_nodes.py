...
@node("Scene to Mocap Prompt (AI)", category="prompt",
      ...
      description="Describe the scene in plain words (Korean or English). A local LLM applies the h3-mocap-prompting skill and writes the full H3 contract: fixed 3/4 camera, one person, timed beats, end hold. Wire 'seconds' into the conditioning so both agree.")
def scene_to_prompt(ctx, scene, seconds, manual_prompt=""):
    if manual_prompt and manual_prompt.strip():
        text = manual_prompt.strip()
    else:
        text = ctx.rewriter.rewrite(scene, float(seconds))
    ctx.ui(text=text)
