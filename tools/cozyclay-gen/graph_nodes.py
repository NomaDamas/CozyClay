...
@node("inputs", category="input", inputs=[inp("image_a", "IMAGEFILE", default="", placeholder="이미지 A (필수)"), inp("image_b", "IMAGEFILE", default="", placeholder="이미지 B (선택)"), inp("ref_video", "VIDEOFILE", default="", placeholder="참고 영상 · 실험 중")], outputs=[("image_a", "IMAGE"), ("image_b", "IMAGE"), ("ref_video", "VIDEOFILE")])
...
@node("scene", category="prompt", inputs=[inp("scene", "STRING", default="", multiline=True, placeholder="예: 인물이 천천히 고개 숙여 인사하고 다시 정면을 바라본다"), inp("seconds", "FLOAT", default=5.0, min=2, max=15, step=1)], outputs=[("prompt", "STRING"), ("seconds", "FLOAT")])
def simple_scene(ctx, scene, seconds):
    return scene, float(seconds)

@node("result", category="output", inputs=[inp("image_a", "IMAGE", optional=True), inp("image_b", "IMAGE", optional=True), inp("ref_video", "VIDEOFILE", optional=True), inp("prompt", "STRING"), inp("seconds", "FLOAT", default=5.0)], outputs=[])
