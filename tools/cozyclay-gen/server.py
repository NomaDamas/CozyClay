...
async def rewrite(request):
    ...
    t0 = time.time()
    try:
        text = await loop.run_in_executor(None, lambda: _rw.rewrite(scene, seconds))
    except Exception as e:
...
def main():
    ...
    if _rw.backend == "local":
        try:
            _rw._load_local()  # ~16 GB bf16 next to the 38 GB of H3 weights
        except Exception:
            log.exception("rewriter LLM failed to load; Scene node will fail until fixed")
