...
function fixture() {
 ...
 const live=ref({characters:chars,objects:[],rigs,shots:[],scenes:[{id:'scene',name:'Fixture'}],activeCharacterId:a.id,stage:{shotAspect:'16:9'},timeline:{currentFrame:0,frameCount:48},filmback:{sensorId:'fullFrame',aspectRatio:16/9},studioSelection:{kind:'character',id:a.id},studioShotId:null,studioView:{mode:'scene',frame:0,playing:false,lookThrough:false,grid:false,autoColor:false}});
 ...
 const ports={revision,read:actual.readStudioState,bounds:actual.studioBounds,commit:actual.commitStudioDraft,commitMotion:actual.commitStudioMotion,operate:actual.operateStudio,loadArtifact:(...args)=>artifactLoader(...args),
 ...
 return {setArtifactLoader:loader=>{artifactLoader=loader;},binding,actual,scope,ports,request,call,motionRequest,revision,semantic,live,store,history,characterRef,buffer,rigs,host,dispose:()=>binding.dispose()};
}
