...
const typeKeys = { text: ["prompt"], image: ["prompt", "image_url"], video: ["prompt", "video_url", "duration"], audio: ["prompt", "audio_url", "duration"], api: ["params"], "video-combiner": ["videos_list", "aspect_ratio"], concat: ["template"], "motion-input": ["characterId"] };

export function createCanvasCommands({ store, makeNode, nodeSchemas }) {
	const history = [];
	const maxHistory = 50;
	const snapshot = () => clone(store.getGraph());
	const mutate = (fn) => { const before = snapshot(); const next = fn(snapshot()); history.push(before); if (history.length > maxHistory) history.shift(); store.setGraph(next); return next; };
	...
	const handlers = {
		get_graph: () => { const graph = clone(store.getGraph()); return { ...graph, nodes: graph.nodes.map((node) => ({ id: node.id, type: node.type, model: node.data?.model || null, data: node.data, position: node.position })), outputs: Object.fromEntries(graph.nodes.map((node) => [node.id, node.data?.outputs || []])) }; },
		add_node: ({ type, model, data = {}, position } = {}) => { const next = mutate((graph) => {
			...
			const node = makeNode(type, id, position || { x: 80 + graph.nodes.length * 40, y: 100 }, modelValue || null, nodeSchemas);
		...
		connect: ({ source, target, sourceHandle = "output", targetHandle = "input" } = {}) => { const next = mutate((graph) => { if (!findNode(graph, source) || !findNode(graph, target)) throw new Error("Unknown node"); if (sourceHandle !== "output") throw new Error("Invalid source handle"); if (targetHandle !== "input" && !String(targetHandle).startsWith("character:") && targetHandle !== "asset" && targetHandle !== "motion") throw new Error("Invalid target handle"); if (graph.edges.some((edge) => edge.source === source && edge.target === target && edge.targetHandle === targetHandle)) throw new Error("Nodes are already connected"); const edge = { id: `e-${source}-${target}-${Date.now()}`, source, target, sourceHandle, targetHandle, animated: true, style: { stroke: "#8994ff", strokeWidth: 2 } }; graph.edges.push(edge); return graph; }); return { edge: next.edges.at(-1), graph: next }; },
		...
		run_workflow: async () => { const result = await store.run(); const next = result?.graph || result; if (next?.nodes) store.setGraph(next); const outputs = Object.fromEntries((next?.nodes || []).map((node) => [node.id, node.data?.outputs || []])); return { outputs, graph: clone(next) }; },
		...
		focus_node: ({ id } = {}) => { if (!findNode(store.getGraph(), id)) throw new Error(`Unknown node: ${id}`); store.focus(id); return { id }; },
		undo: () => { const previous = history.pop(); if (!previous) return { undone: false }; store.setGraph(previous); return { undone: true, graph: clone(previous) }; },
		history,
	};
